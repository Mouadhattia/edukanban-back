...
const createUser = async (req, res) => {
  try {
    ...
    // Prepare user data with proper array fields
    const userData = {
      email,
      password, // Password will be automatically encrypted by User model pre('save') middleware
      role,
      fullName,
      isEmailVerified: true,
    };

    // Add proper organization or school association as arrays
    if (schoolId) {
      userData.schoolIds = [schoolId];
    }
    if (organizationId) {
      userData.organizationIds = [organizationId];
    }
    if (status) {
...
const updateUser = async (req, res) => {
  try {
    ...
    // Handle schoolId - convert to array format
    if (schoolId) {
      user.schoolIds = [schoolId];
    }

    // Handle organizationId - convert to array format
    if (organizationId) {
      user.organizationIds = [organizationId];
    }

    // Handle password - will be automatically encrypted by User model pre('save') middleware
    if (password) {
...
const getStandard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching standard", error: error.message });
  }
};
...
const createStandard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error creating standard", error: error.message });
  }
};
...
const updateStandard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error updating standard", error: error.message });
  }
};
...
const deleteStandard = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting standard", error: error.message });
  }
};
...
const getUserDistribution = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Error fetching user distribution data",
      error: error.message,
    });
  }
};
