...
const registerValidation = [
  ...
  body("invitationCode").custom((value, { req }) => {
    ...
    if (req.body.role === ROLES.SCHOOL_ADMIN) {
      if (
        ...
        throw new Error(
          "School details are required for school admin registration"
        );
      }
      return true;
    }
    if (req.body.role === ROLES.ORGANIZATION_ADMIN) {
      if (!req.body.organizationName || !req.body.organizationType) {
        throw new Error(
          "Organization details are required for organization admin registration"
        );
      }
      return true;
    }
